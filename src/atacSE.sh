...
then
    ...
    echo "**********************************************************************"
    echo ""
    echo "Usage: $0 <hg38|hg19|mm10> <read1.fq.gz> <genome.fa> <output prefix>"
    echo ""
    exit -1
fi
...
HG=${3}
OUTP=${4}
RESUME=${5} #step to resume at, steps le RESUME will be run
if [ -z $RESUME ]; then RESUME=5000; fi
...
echo STEP1 - align
# Align
if [ ! -f ${OUTP}.final.bam ] || [ $RESUME -le 1 ]; then
  ${BASEDIR}/alignSE.sh ${ATYPE} ${FQ1} ${HG} ${OUTP}
else
...
if [ ! -f ${OUTP}.pseudorep1.bam ] || [ ! -f ${OUTP}.pseudorep2.bam ]  || [ $RESUME -le 2 ]; then
...
if [ ! -f ${OUTP}.final.peaks ]  || [ $RESUME -le 3 ]; then
  ${BASEDIR}/peaksSE.sh ${OUTP}.pseudorep1.bam ${OUTP}.pseudorep2.bam ${HG} ${OUTP} ${ATYPE}
else
...
if [ ! -f ${OUTP}.key.metrics ]  || [ $RESUME -le 5 ]; then
...
if [ ! -f ${OUTP}.norm.filtered.vcf.gz ]  || [ $RESUME -le 6 ]; then 
...
if [ ! -s  ${OUTP}.annotated.normalized ]  || [ $RESUME -le 7 ]; then
...
# Motif discovery
echo STEP 8 - motifs
if [ ! -d ${OUTP}_motifs ]  || [ $RESUME -le 8 ]; then

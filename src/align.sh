...
# Adapter trimming
if [ ! -f ${OUTP}.1.fq.gz ] || [ ! -f ${OUTP}.2.fq.gz ]; then
  cutadapt -q 10 -m 15 -e 0.10 -a CTGTCTCTTATA -A CTGTCTCTTATA -o ${OUTP}.1.fq -p ${OUTP}.2.fq ${FQ1} ${FQ2} | gzip -c > ${OUTP}.cutadapt.log.gz
  gzip ${OUTP}.1.fq
  gzip ${OUTP}.2.fq
else
  echo ${OUTP}.1.fq.gz and ${OUTP}.2.fq.gz found, assuming cutadapt succeeded. Delete to rerun.
fi
...
if [ ! -f ${OUTP}.raw.bam ]; then
  bowtie2 --threads ${THREADS} --local --maxins 2000 -x ${HG} -1 ${OUTP}.1.fq.gz -2 ${OUTP}.2.fq.gz 2> ${OUTP}.bowtie.log | samtools view -bT ${HG}.fa - > ${OUTP}.raw.bam
else
